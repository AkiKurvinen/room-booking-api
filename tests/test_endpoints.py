...
def test_create_overlapping_booking(client):
    ...
    overlapping_start = tomorrow.replace(hour=11, minute=0, second=0, microsecond=0).isoformat()
    overlapping_end = tomorrow.replace(hour=13, minute=0, second=0, microsecond=0).isoformat()
    response = client.post(
        f"{API_PREFIX}/rooms/1/bookings/",
